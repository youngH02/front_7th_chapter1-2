...
- **⚠️ 모든 테스트 코드 완전 작성 필수 (절대 금지 사항)**: RED 단계에서는 모든 테스트가 완전히 구현되어야 합니다
  - **TODO 주석 절대 금지**: `// TODO: 구현 예정`, `// TODO:` 등의 주석은 절대 사용할 수 없습니다
  - **Placeholder 테스트 코드 절대 금지**: `expect(true).toBe(true)`, `expect(true).toBe(false)` 등 형식적인 테스트 코드는 절대 사용할 수 없습니다
  - **모든 테스트는 완전히 구현되어야 함**: 각 테스트는 실제 함수 호출, 입력 데이터, 검증 로직이 모두 포함된 완전한 코드여야 합니다
  - 테스트 코드가 미완성이면 RED 단계를 완료할 수 없습니다
- 실제 함수 호출과 검증 로직을 작성하여 의미 있는 실패(RED)를 만들어야 함
...
5. **테스트 구조 구성**: 설계 문서의 범위(unit/integration/system)에 따라 디렉터리와 `describe` 블록을 구성합니다.
...
8. **주요 규칙 점검**: 다음 항목들을 확인합니다
   - TD-### 번호 포함 여부
   - 정상/예외/경계/부정 케이스 반영 여부
   - **⚠️ 완전성 검증**: TODO 주석이나 placeholder 테스트 코드(`expect(true).toBe(true)` 등)가 없는지 확인
   - 모든 테스트가 완전한 코드로 작성되었는지 확인
   - 명세 참조 주석 등

### C. 커버리지 확인 & 보완 (9~11)

9. **코드 매칭 표 생성**: 설계 문서의 테스트 목록과 실제 코드의 `it()` 블록을 비교해 매칭 여부를 표로 작성합니다.
10. **결과 노출 및 보완**: 매칭 표를 화면에 표시합니다. 누락 항목이 있으면 즉시 테스트를 추가한 뒤 9~10단계를 반복해 100% 매칭을 달성합니다. 사용자 확인 없이 자동으로 누락 항목을 보완하여 모든 테스트를 작성합니다.

- **⚠️ 완전성 재검증**: 누락 항목 추가 후에도 TODO 주석이나 placeholder 테스트 코드가 있는지 다시 확인합니다
- 모든 테스트가 완전한 코드로 작성되었는지 확인합니다
- 완전성 검증이 통과되면 다음 단계로 진행합니다
...
| ---- | ------ | -------------------------- | --------------------------------- | ------------------- | ---- |
| 1    | TD-001 | 일정 추가 - 기본 정상 흐름 | `src/__tests__/integration/...ts` | ✅ 완료             |      |
| 2    | TD-002 | 일정 수정 - 제목/시간 변경 | `src/__tests__/integration/...ts` | ✅ 완료             |      |
| 3    | TD-010 | 필수 항목 미입력 오류 처리 | `src/__tests__/unit/...spec.ts`   | ✅ 완료             |      |
| ...  | ...    | ...                        | ...                               | ...                 | ...  |
...
- ✅ 설계 문서 대비 테스트 매칭 100% 확인
- ✅ **모든 테스트 코드 완전 작성 확인**: TODO 주석이나 placeholder 테스트 코드가 없는지 확인
...
- 커밋 완료 후 자동으로 다음 단계(STEP04)로 진행합니다
