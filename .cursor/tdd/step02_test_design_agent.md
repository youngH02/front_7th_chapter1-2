...
6. **초기 목록 공유**: 설계된 전체 테스트 케이스 표와 명세-테스트 매칭 표를 화면에 표시하고 사용자 피드백을 기다립니다. 요청된 수정·추가를 반영합니다.

### C. 커버리지 검증

7. **커버리지 매트릭스 작성**: 명세 항목별로 네 가지 케이스 유형이 충족되는지 표로 정리합니다.
8. **누락 검토 및 사용자 확인**: 누락 항목이 있으면 명세-테스트 매칭 표에 반영하여 화면에 표시하고, 추가 설계 진행 여부를 사용자에게 질문합니다. 추가가 결정되면 테스트를 보완하고 7~8단계를 반복합니다.
9. **통계 계산**: 케이스 유형/우선순위/범위 분포를 계산해 문서에 포함합니다.

### D. 문서화 및 요약

10. **최종 목록 공유**: 보완된 테스트 케이스 표·통계·명세-테스트 매칭 표를 다시 보여주고 사용자 최종 승인을 받습니다.
11. **문서 작성**: 아래 템플릿을 사용해 테스트 설계 문서를 작성합니다.
12. **파일 저장**: `docs/test_design/###_기능명_test_design.md` 형식으로 파일 저장
13. **⚠️ STEP02 요약 화면 표시 (절대 필수)**: 문서 저장 후 반드시 STEP02 요약을 화면에 표 형식으로 표시
    - "STEP02 요약 (반드시 화면에 표시!)" 섹션의 형식을 정확히 따라 표시
    - STEP02 요약 정보 표 (테스트 설계 문서, 총 테스트 수, 우선순위 분포, 커버리지 상태, 명세 매칭 상태)
    - 테스트 TODO 목록 표 (ID, 유형, 우선순위, 요약)
    - 구현 시 주의사항 (리스크, 힌트)
    - 다음 단계 안내
    - **STEP02 요약을 표시하지 않으면 STEP02를 종료하지 않음**
...
### 2. STEP02 요약 (반드시 화면에 표시!)

**⚠️ 필수**: 문서 저장 후 반드시 아래 형식으로 화면에 표시하여 사용자가 확인할 수 있도록 해야 합니다!
...
| 항목             | 내용                                         |
| ---------------- | -------------------------------------------- |
| 테스트 설계 문서 | docs/test_design/###\_feature_test_design.md |
| 총 테스트 수     | [N]개                                        |
| 우선순위 분포    | High: [A]개, Medium: [B]개, Low: [C]개       |
| 커버리지 상태    | ✅ 100% / 누락 없음 (또는 ⚠️ 일부 누락)      |
| 명세 매칭 상태   | ✅ 모든 명세 번호가 TD에 매핑됨              |

## 테스트 TODO 목록 (요약)

| ID     | 유형        | 우선순위 | 요약   |
| ------ | ----------- | -------- | ------ |
| TD-001 | 정상 케이스 | High     | [요약] |
| TD-010 | 예외 케이스 | High     | [요약] |
| TD-020 | 경계 케이스 | Medium   | [요약] |
| ...    | ...         | ...      | ...    |

**전체 테스트 목록은 설계 문서를 참고하세요.**
...
```
chore(td-design): add test design for [기능명]

- 총 테스트 케이스: [N]개
- High: [A]개 / Medium: [B]개 / Low: [C]개
- 설계 문서: docs/test_design/###_[slug]_test_design.md
```
...
- `run_terminal_cmd` 도구를 사용하여 다음 명령을 실행:
  ...
  git commit -m "chore(docs): add test design for [기능명]" -m "- 총 테스트 케이스: [N]개" -m "- High 우선순위: [A]개, Medium: [B]개, Low: [C]개" -m "- 테스트 설계 문서: docs/test_design/###_기능명_test_design.md"
  ```
