...
11. **⚠️ STEP02 요약 화면 표시 (절대 필수)**: 사용자에게 STEP02 요약을 화면에 표 형식으로 **먼저** 표시
    ...
    - 구현 시 주의사항 (리스크, 힌트)
    - 다음 단계 안내
    - **STEP02 요약을 표시하고 사용자가 확인할 때까지 다음 단계로 진행하지 않음**
12. **문서 작성**: 사용자가 STEP02 요약을 확인한 후, 아래 템플릿을 사용해 테스트 설계 문서를 작성합니다.
13. **파일 저장**: `docs/test_design/###_기능명_test_design.md` 형식으로 파일 저장
14. **⚠️ 커밋 실행 (절대 필수)**: 파일 저장 후 반드시 커밋 실행
    - STEP02 요약을 사용자가 확인한 후 문서를 저장하고 커밋 수행
...
**⚠️ 필수**: 문서 저장 **전에** 반드시 아래 형식으로 화면에 표시하여 사용자가 먼저 확인할 수 있도록 해야 합니다!
